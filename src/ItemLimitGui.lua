--[[
  This is part of the network view.
  It shows a simple GUI used to select an item limit.
...
-- open the pop-up
function M.open_modal(player, item, temp, count)

  local ui = GlobalState.get_ui_state(player.index)
  if ui.net_view.modal ~= nil then
...
-- close the modal window and revert back to the main net view frame
function M.close_modal(player)
  local ui = GlobalState.get_ui_state(player.index)
  if ui.net_view == nil then
    return
  end
  local modal = ui.net_view.modal
  if modal == nil then
    return
  end
  modal.frame.destroy()
  ui.net_view.modal = nil
  player.opened = ui.net_view.frame
end
