...
local function build_item_page(parent, add_refresh)
  ...
  add_refresh = false
  if add_refresh ~= false then
    local header_flow = main_flow.add({
      type = "flow",
      direction = "horizontal",
    })

    -- REVISIT: center refresh button?
    header_flow.add({
      type = "button",
      caption = "Refresh",
...
function M.close_main_frame(player_index)
...
function M.open_main_frame(player_index)
  ...
  I want the ||||| stuff to make the window draggable and the X to close.
  Right now, I can get it to look right, but it isn't draggable.
  OR I can omit the [X] and make it draggable. Sigh.
  ]]
  ...
  local frame = player.gui.screen.add({
    ...
    -- enabling the caption adds the draggable stuff that works, but doesn't allow the close button to be on the title line
  ...
  if true then
    ...
    header_flow.add{
      ...
      style = "close_button",
      --tooltip = { "gui.close-instruction" }, -- part of the "close_button" style
  ...
  tabbed_pane.add_tab(tab_item, build_item_page(tabbed_pane, true))
  tabbed_pane.add_tab(tab_fluid, build_item_page(tabbed_pane, true))
  tabbed_pane.add_tab(tab_shortage, build_item_page(tabbed_pane, true))
...
local tab_idx_to_view_type = {
  "item", "fluid", "shortage", "limits"
}
local view_type_to_tab_idx = {}
for idx, view in ipairs(tab_idx_to_view_type) do
  view_type_to_tab_idx[view] = idx
end
