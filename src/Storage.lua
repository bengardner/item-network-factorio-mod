--[[
This file abstracts the storage table used for items and fluids for a force.
Uses DomainStore.
]]
local Storage = {}
...
-- max is based on 32-bit signed number used with comparators
...
-- entity names that cannot be stored
...
local function default_storage(domain_key)
  return {
    -- key=item_name, val=amount
    -- key=fluid_name, val={ key=temp, val=amount }
    items = {},
    -- key=item_or_fluid_name, val=amount
    limits = {},
    reservations = {},
...
local function get_storage(entity)
  return DomainStore.get_subdomain(DomainStore.get_domain_key(entity), "storage", default_storage)
end

--[[
Grab the storage based on the entity surface and force.
Sets storage.last_entity to the entity so it doesn't need to be passed around.
]]
function Storage.get_storage(entity)
  local storage = get_storage(entity)
  storage.last_entity = entity
  return storage
end

--[[
Grab the storage for a force on a different surface.
]]
